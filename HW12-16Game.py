...
S = {}
myFile= open(resource_path('Slots16.csv'))
myReader = csv.reader(myFile)
for row in myReader:
    for cell in row:
    ...
    for a in T:
        if h != a:
            ...
            if h_division != a_division:
                for s in S[17]:  # All slots for Week 18 (outside of the `if` condition)
                    # Check if myGames[h, a, s, 18] exists before creating a constraint
                    if (a, h, s, 17) in myGames:
                        constrName = f'same_division_week_18_{a}_vs_{h}_slot_{s}'
                        myConstr[constrName] = myModel.addConstr(myGames[a, h, s, 17] == 0, name=constrName)
                    #else:
...
for t in T:  # Iterate over teams
    for w in range(1, 17):  # Iterate over weeks (up to week 16 to avoid out of range error)
